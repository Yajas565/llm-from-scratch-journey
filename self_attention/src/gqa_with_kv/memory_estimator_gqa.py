...
def calc_kv_bytes_total(batch, context_length, emb_dim, num_heads, num_kv_heads, n_layers, bytes_per_elem):
    head_dim = math.ceil(emb_dim/num_heads)
    per_layer = batch * context_length * head_dim * num_kv_heads * 2 * bytes_per_elem
...
def main():
    ...
    if (cfg['n_heads'] % cfg['n_kv_groups']) != 0:
        raise ValueError(f"n_kv_groups must divide n_heads exactly")
